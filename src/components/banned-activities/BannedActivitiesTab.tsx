...
export function BannedActivitiesTab() {
  ...
  const breakMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['bannedActivities'] });
      toast('Logged 💔', { description: t('banned.logged_desc') });
    },
    ...
    <div className="space-y-6">
      ...
      {(activities as any[]).length > 0 && (
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: t('banned.total_banned'), value: (activities as any[]).length, icon: ShieldOff, color: 'text-red-400' },
            { label: t('banned.times_broken'), value: totalBroken, icon: Flame, color: 'text-orange-400' },
            { label: t('banned.high_severity'), value: highSeverity, icon: AlertTriangle, color: 'text-yellow-400' },
          ].map(s => (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder={t('banned.search_placeholder')}
        ...
        <div className="flex gap-2">
          <select value={severityFilter} onChange={e => setSeverityFilter(e.target.value)}
            ...
            <option value="">{t('banned.filter_all')}</option>
          ...
          <button onClick={() => setShowModal(true)}
            ...
            <span className="hidden sm:inline">{t('banned.add_btn')}</span>
          </button>
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <h3 className="text-xl font-bold text-white mb-2">
            {search || severityFilter ? t('banned.no_match') : t('banned.no_activities')}
          </h3>
          <p className="text-gray-600 text-sm max-w-xs mx-auto mb-6">
            {search || severityFilter ? t('banned.no_match_desc') : t('banned.no_activities_desc')}
          </p>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Flame className="w-3.5 h-3.5" /> {t('banned.i_broke_this')}
                    </button>
