// GET / PATCH / DELETE /api/goals/monthly/[id]
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.monthlyGoal.findFirst({ where: { id: params.id, userId: auth.sub } });
  ...
  if (parsed.data.completed === true && !existing.completed) {
    ...
    await prisma.activityLog.create({
      data: { userId: auth.sub, type: 'MONTHLY_GOAL_COMPLETED', entityId: params.id, xpDelta: xp },
    });
  }
...
export async function DELETE(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const existing = await prisma.monthlyGoal.findFirst({ where: { id: params.id, userId: auth.sub } });
  if (!existing) return err('Goal not found', 404);
  await prisma.monthlyGoal.delete({ where: { id: params.id } });
